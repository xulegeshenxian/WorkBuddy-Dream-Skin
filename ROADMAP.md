...
### H. 挂件系统重做（用户 2026-07-19 反馈：单调 + 五套皮肤共用一个拍立得模板）
...
- [x] **light 主题下 markdown 代码块文字看不见**（pink-dream / sunlit-campus / mint-botanical 全中）。`20-workspace-preparing.css:48, 61` 里给 `.cb-markdown-pre` 和 `.cb-markdown-pre code` 硬编码 `color: #e5f5ef !important`（深海调的浅薄荷）。light 主题上 `--wbds-background` 是近白，`--wbds-text` 是深色（如 pink-dream 的 `#4D2636`），但硬编码把文字强制成浅薄荷 → 浅薄荷叠 96% 近白 = 1:1 对比度，肉眼几乎不可见。修：两处 `#e5f5ef` 都换 `var(--wbds-text)`，token 层自动切明暗（`ecd95d7`, 2026-07-19）
