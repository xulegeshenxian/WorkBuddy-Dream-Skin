...
| **导入或更换图片** | 选一张图片，脚本自动分析色调 + 从图取色生成主题 |
| **保存当前主题为预设...** | 把当前生效的主题以你起的名字保存到主题库，从此可切换回来 |
...
```powershell
.\scripts\customize-workbuddy-theme.ps1 -ImagePath "你的图" -Style MintBloom
```
`-Style` 支持 `PinkDream` / `MintBloom` / `SunlitCampus` / `GildedNight` / `DeepSea` / `Auto`。

### 把导入的主题保存下来

「导入或更换图片」只是**临时应用**——切到别的主题就没了。想留下：

托盘 → **保存当前主题为预设...** → 弹窗输入 id（比如 `my-sakura`，只允许 `a-z / 0-9 / 连字符`，长度 3-40）→ 回车 → 主题库里就多了一套。

以后**切换主题 ▸** 子菜单里就能看到 `my-sakura` 排在内置主题后面，随时切回。

同名会弹二次确认，防止误覆盖。想覆盖旧的自定义主题就选"是"。
