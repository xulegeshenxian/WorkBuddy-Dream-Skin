...
* 修复专家、技能、连接器弹层中透明埋点按钮在悬浮时变成不透明背景并遮住文字的问题。
* 同步修复使用相同菜单结构的更多弹层。
* 悬浮审计新增专家、技能、连接器三行真实鼠标移动测试，校验行背景、标签可见性、覆盖按钮透明度和文字对比度。
* 全页面审计新增侧边栏专家弹层悬浮状态和截图。
* 历史任务审计会从最多八个候选中抽取四个有正文的会话，空会话会记录为跳过项；若全部候选均未加载消息，则明确标记整组未测试，避免把服务端未返回消息误判为皮肤失败。

## 0.7.5

### 悬停态完整性修复

* 修复自动化任务行悬停时变成浅灰底并导致文字消失的问题。
* 补齐历史任务输入区的模型按钮、更多操作按钮和圆形工具按钮悬停状态。
* 修复添加文件、模式、技能等输入区级联菜单的激活项背景被透明重置覆盖的问题。
* 补齐没有菜单角色的 Plan 与 Ask 模式行悬停状态。
* 扩展悬停和页面审计，覆盖自动化任务行、历史任务输入区与多级操作菜单。

## 0.7.4

### WorkBuddy 5.3.3 兼容更新

* 适配侧栏任务搜索、筛选按钮与任务分区标签，消除新增控件的白底和低对比文字。
* 适配专家、技能、连接器顶部标签、搜索框、操作按钮和侧栏子菜单。
* 适配新版输入区模型按钮、工作空间、权限按钮、模型列表、等级标签与悬停状态，选择器不再依赖旧版构建哈希。
* 适配相关玩法卡片、翻页按钮、通用灰色按钮、自动化连接器选择器与提示条。
* 修复设置中心存储卡、本地记忆卡、助理昵称、标题、标签和下拉箭头的亮色残留。
* 设置审计同步当前 11 个分区，移除 WorkBuddy 5.3.3 已取消的智能体邮箱入口。
* 场景审计延长异步内容等待，并兼容快捷入口直接跳转目标页的行为。
* 历史任务审计延长首屏稳定等待，减少大型任务首次加载造成的假失败。
* 修复浅色主题的强调色前景、辅助文字、用户菜单升级按钮和自动化提示徽标对比度。
* 调整白瓷花房与琥珀夜灯的辅助文字色，确保浅色背景上的正文对比度达到审计阈值。
* 修复连续批量审计时主题切换脚本继承上一条外部命令退出码的问题，成功切换现在始终返回零。
* 输入区权限按钮改用正文色，避免绯樱少女等浅色主题的强调色文字低于 4.5 对比度阈值。
* 新增明暗自适应的交互文字令牌，浅色主题的悬停与选中状态使用正文色，深色主题继续保留高亮强调色。
* 深层页面、设置、弹窗和业务标签统一接入交互文字令牌，并提高绯樱少女辅助文字在粉色面板上的对比度。
